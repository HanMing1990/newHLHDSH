...
//心率通知key，通过Notificaiton的object来传输心率值
#define kAppscommHeartRateNotificaiton @"AppscommHeartRateNotificaitonKey"
 ...
 *  OTA升级固件
 *
 *  @param fileContent Hex文件内容
 *  @param completion
 */
- (void)updateFirmwareThroughOTAWithData:(NSString *)fileContent completion:(AppscommUpdateFirmwareCallback)completion;

/**
 * 设置自动心率监测
 *
 * @param period     自动心率开启时间间隔,暂定有效值为:0分钟（关闭）、5分钟、15分钟、30分钟、45分钟
 * @param completion
 */
- (void)writeAutomaticHeartRateMonitorPeriod:(Byte)period completion:(AppscommWriteSettingsCallback)completion;
 ...
 * @param completion
 */
- (void)writeHeartrateRangeAlarmStatus:(BOOL)status completion:(AppscommWriteSettingsCallback)completion;

/**
 * 设置自动心率范围上限
 *
 * @param limitValue     上限值
 * @param completion
 */
- (void)writeHeartrateRangeHighLimit:(Byte)limitValue completion:(AppscommWriteSettingsCallback)completion;

/**
 * 设置自动心率范围下限
 *
 * @param limitValue     下限值
 * @param completion
 */
- (void)writeHeartrateRangeLowLimit:(Byte)limitValue completion:(AppscommWriteSettingsCallback)completion;
 ...
 * @param completion
 */
- (void)readHeartRateData:(AppscommReadHeartRateDataCallback)completion;
@end