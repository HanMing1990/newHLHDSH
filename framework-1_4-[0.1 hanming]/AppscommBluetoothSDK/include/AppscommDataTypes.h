...
typedef void(^AppscommUpdateFirmwareCallback)(float progress, NSError *error);
/**
 *  获取提醒
 *
 ...
 * 获取心率数据
 *
 * @param heartrateData 心率数据， AppscommHeartRate
 */
typedef void(^AppscommReadHeartRateDataCallback)(NSArray *heartrateData, NSError *error);
 ...
 * 心率类型
 */
@interface AppscommHeartRate : NSObject
@property (nonatomic, assign) NSUInteger flag;//0x00：停止, 0x01：自动, 0x02：手动, 0x03：心率数据上传状态
@property (nonatomic, assign) NSUInteger timestamp;//时间戳
@property (nonatomic, assign) NSUInteger heartrate;//心率值
@end
